...
void publicarMQTT(float temperatura) {
  ...
  mqtt.publish(
    "granja/galpao1/sensores/temperatura",
    tempStr
  );

  mqtt.publish(
    "granja/galpao1/reles/rele1",
    estadoRele1 ? "ON" : "OFF"
  );

  mqtt.publish(
    "granja/galpao1/reles/rele2",
    estadoRele2 ? "ON" : "OFF"
  );

  mqtt.publish(
    "granja/galpao1/reles/rele3",
    estadoRele3 ? "ON" : "OFF"
  );

  mqtt.publish(
    "granja/galpao1/alarmes/falhaSensor",
    falhaSensor ? "TRUE" : "FALSE"
  );

  mqtt.publish(
    "granja/galpao1/alarmes/temperaturaAlta",
    alarmeTemperatura ? "TRUE" : "FALSE"
  );
...
void controlarRele(
  ...
  if (!estadoAtual && temperatura >= tempLigar) {
    ...
    estadoAtual &&
    temperatura <= (tempLigar - HISTERESIS)
  ) {
